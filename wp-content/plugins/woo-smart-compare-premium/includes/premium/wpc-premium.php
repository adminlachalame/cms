		...
		if ( ! wpc_get_update_key( 5570 ) ) {
			...
			<div data-dismissible="woosc_update" class="wpc-notice notice notice-warning is-dismissible">
				<p>Please verify <a href="<?php echo admin_url( 'admin.php?page=wpclever-keys' ); ?>">License Key</a> of
					<strong>WPC Smart Compare</strong> to enjoy unlimited update release and get the latest plugin update directly on the website backend.
				</p>
			</div>
			<?php
		}
	...
	class WPCleverPremium {
		...
		function admin_enqueue_scripts( $hook ) {
			if ( strpos( $hook, 'wpclever-keys' ) ) {
				wp_enqueue_style( 'wpc-premium', WOOSC_URI . 'includes/premium/css/premium.css' );
				wp_enqueue_script( 'wpc-premium', WOOSC_URI . 'includes/premium/js/premium.js', [ 'jquery', ] );
			}
		}
