													...
													<div class="woosc-fields-more">
														<select class="woosc-field-types">
															...
															<optgroup label="<?php esc_attr_e( 'Custom', 'woo-smart-compare' ); ?>">
																...
																<option value="custom_attribute" data-type="custom_attribute"><?php esc_html_e( 'Custom attribute', 'woo-smart-compare' ); ?></option>
																<option value="custom_field" data-type="custom_field"><?php esc_html_e( 'Custom field', 'woo-smart-compare' ); ?></option>
															</optgroup>
														</select>
						...
						foreach ( $products as $product_id ) {
							...
							foreach ( $fields as $key => $field ) {
								...
								if ( $field_type === 'attribute' ) {
									$product_attribute = $product->get_attribute( $field_name );

									if ( empty( $product_attribute ) && $parent_product ) {
										$product_attribute = $parent_product->get_attribute( $field_name );
									}

									$products_data[ $product_id ][ $field_name ] = apply_filters( 'woosc_product_' . $field_name, $product_attribute, $product );
								}

								if ( $field_type === 'custom_attribute' ) {
									$custom_attribute_name = trim( $field_name );
									$custom_attribute_ca   = 'ca_' . sanitize_title( $custom_attribute_name );
									$custom_attribute_val  = $product->get_attribute( $custom_attribute_name );

									if ( empty( $custom_attribute_val ) && $parent_product ) {
										$custom_attribute_val = $parent_product->get_attribute( $custom_attribute_name );
									}

									$products_data[ $product_id ][ $custom_attribute_ca ] = apply_filters( 'woosc_product_' . $custom_attribute_ca, implode( ', ', explode( ' | ', $custom_attribute_val ) ), $product );
								}

								if ( $field_type === 'custom_field' ) {
									$custom_field_name = trim( $field_name );
									...
									if ( empty( $custom_field_val ) && $parent_product && ( $parent_product_id = $parent_product->get_id() ) ) {
										$custom_field_val = get_post_meta( $parent_product_id, $custom_field_name, true ) ?: get_post_meta( $parent_product_id, sanitize_title( $custom_field_name ), true );
									}

									if ( ( strpos( $custom_field_cf, 'cf_wpcf' ) !== false ) && is_array( $custom_field_val ) && ! empty( $custom_field_val ) ) {
									...
									$products_data[ $product_id ][ $custom_field_cf ] = apply_filters( 'woosc_product_' . $custom_field_cf, $custom_field_val, $product );
								}
